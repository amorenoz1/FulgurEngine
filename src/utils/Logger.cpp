...
void Utils::Logger::Log(LoggingPriority priority, const std::string& message, const char* file,
    ...
    if (m_Config.format == LoggingFormat::EXTENDED && function) {
        oss << "[" << function << "] ";
    }
...
std::string Utils::Logger::FormatExecutionTime(float seconds) {
...
std::string Utils::Logger::PriorityToString(LoggingPriority p) {
    switch (p) {
...
std::string Utils::Logger::Colorize(LoggingPriority p, const std::string& msg) {
